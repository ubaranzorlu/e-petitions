  ...
  describe "notifying signature of a government response" do
    ...
    shared_examples_for "a government response email" do
      it "includes a link to the petition page" do
        expect(mail).to have_body_text(%r[https://demokra.si/archived/petitions/#{petition.id}])
      end
      ...
      it "includes an unsubscribe link" do
        expect(mail).to have_body_text(%r[https://demokra.si/archived/signatures/#{signature.id}/unsubscribe\?token=#{signature.unsubscribe_token}])
      end

      it "has a List-Unsubscribe header" do
        expect(mail).to have_header("List-Unsubscribe", "<https://demokra.si/archived/signatures/#{signature.id}/unsubscribe?token=#{signature.unsubscribe_token}>")
      end
      ...
      it "includes a link to read the response online" do
        expect(mail).to have_body_text(%r[https://demokra.si/archived/petitions/#{petition.id}\?reveal_response=yes])
      end
